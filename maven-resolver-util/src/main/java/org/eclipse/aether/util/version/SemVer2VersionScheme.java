 ...
 * case-insensitively. The following qualifier strings are recognized and treated specially: "alpha" = "a" &lt;
 * "beta" = "b" &lt; "milestone" = "m" &lt; "preview" &lt; "rc" = "cr" &lt; "snapshot" = "dev" &lt; "final" = "ga" &lt;
 * "sp". All qualifiers are considered bigger/newer than other strings. An empty segment/string is equivalent to 0.
 * </p>
 * <p>
 ...
 * </p>
 */
public class SemVer2VersionScheme extends VersionSchemeSupport {

    // Using WeakHashMap wrapped in synchronizedMap for thread safety and memory-sensitive caching
    private final Map<String, SemVer2Version> versionCache = Collections.synchronizedMap(new WeakHashMap<>());
    ...
    static {
        ...
        if (isStatisticsEnabled()) {
            Runtime.getRuntime().addShutdownHook(new Thread(SemVer2VersionScheme::printGlobalStatistics));
        }
    }

    public SemVer2VersionScheme() {
    ...
    @Override
    public SemVer2Version parseVersion(final String version) throws InvalidVersionSpecificationException {
        totalRequests.incrementAndGet();
        GLOBAL_TOTAL_REQUESTS.incrementAndGet();

        SemVer2Version existing = versionCache.get(version);
        if (existing != null) {
            ...
            return versionCache.computeIfAbsent(version, SemVer2Version::new);
        }
    }
    ...
    public String getCacheStatistics() {
        ...
        return String.format(
                "SemVer2VersionScheme Cache Stats: hits=%d, misses=%d, total=%d, hit-rate=%.2f%%, cache-size=%d",
    ...
    private static void printGlobalStatistics() {
        ...
        System.err.println("=== SemVer2VersionScheme Global Cache Statistics (WeakHashMap) ===");
     ...
     * <pre>jbang --main=org.eclipse.aether.util.version.SemVer2VersionScheme org.apache.maven.resolver:maven-resolver-util:1.9.18 "1.2.7" "1.2-SNAPSHOT"</pre>
    ...
    public static void main(String... args) {
        ...
        SemVer2VersionScheme scheme = new SemVer2VersionScheme();
        SemVer2Version prev = null;
        int i = 1;
        for (String version : args) {
            try {
                SemVer2Version c = scheme.parseVersion(version);
